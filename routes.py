...
def departmentmanage():
    ...
    Page Layout (yes it's hacky, and no it's not secure - it's for demonstration only!):
        1. /departmentmanage => Show table of models to departments
        2. /departemntmodels?model=___&manufacturer=___ => show employee counts
        3. /departmentmanage?model=___&manufacturer=___&empid=____ => show all devices for model and indicate.
    """
    # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))
    ...
    if empid != '' and manufacturer != '' and model != '' and department != '':
        ...
        if device_assigned is None:
            ...
            page['bar'] = True
            return redirect(url_for('departmentmanage'))
        ...
        # Show all models and employees in the department
        department_models = database.get_department_models(session['manager'])
        employee_list = database.get_employee_status(session['manager'])

        return render_template('departmentmanage.html',
...
#####################################################
#   Assign avaliable employees to department
#####################################################
...
def assign():
    ...
    # If they're sending the assign
    employee_id = request.args.get('empid')
    department = request.args.get('department')
    manufacturer = request.args.get('manufacturer')
    model = request.args.get('model')

    if department is None or employee_id is None:
        page['bar'] = False
        flash('Invalid Request Sent')
        return redirect(url_for('departmentmanage'))

    # Attempt the "revoke"
    success = database.assign(employee_id, department)
    ...
    page['bar'] = success[0]
    flash("Employee Assigned" if success[0] else success[1])
...
#####################################################
#   Dismiss current employees in department
#####################################################
@app.route('/dismiss', methods=['GET'])
def dismiss():
    if('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))
    ...
    # If they're sending the dismiss
    employee_id = request.args.get('empid')
    department = request.args.get('department')
    manufacturer = request.args.get('manufacturer')
    model = request.args.get('model')
    ...
    # Attempt the "revoke"
    success = database.dismiss(employee_id, department)
    ...
    page['bar'] = success[0]
    flash("Employee dismissed" if success[0] else success[1])
